...
export async function POST(
  ...
  const sp = await prisma.simpleProduct.findUnique({ where: { id } });
  if (!sp) {
    return NextResponse.json(
  ...
  const filename = `simple-${id}-${Date.now()}.webp`;
  await fs.writeFile(path.join(IMG_DIR, filename), webp);

  const previousUrl = sp.imageUrl;
  const imageUrl = `/productos/${filename}`;
  const updated = await prisma.simpleProduct.update({ where: { id }, data: { imageUrl } });
...
export async function DELETE(
  ...
  const previousUrl = sp.imageUrl;
  await prisma.simpleProduct.update({ where: { id }, data: { imageUrl: null } });
  await tryDelete(previousUrl);
