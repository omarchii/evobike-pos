...
export async function POST(
  ...
  const filename = `variante-${id}-${Date.now()}.webp`;
  await fs.writeFile(path.join(IMG_DIR, filename), webp);

  const previousUrl = variant.imageUrl;
  const imageUrl = `/productos/${filename}`;
  const updated = await prisma.productVariant.update({ where: { id }, data: { imageUrl } });
...
export async function DELETE(
  ...
  const previousUrl = variant.imageUrl;
  await prisma.productVariant.update({ where: { id }, data: { imageUrl: null } });
  await tryDelete(previousUrl);
