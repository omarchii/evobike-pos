...
- **Tech-debt lint (snapshot 2026-04-14, pre-Caja P5.6 ampliada)** — `npx eslint src/` reporta **10 errors + 12 warnings** pre-existentes que el React Compiler bloquea en compile-skip (no rompen runtime pero saltan memoización). No arreglar durante Caja; limpiar en batch aquí.
  - **React Compiler purity (7 errors)** — `Date.now()` en render: `dashboard/authorization-inbox.tsx:346`, `workshop/page.tsx:28,219`, `components/pos/authorization/discount-authorization-panel.tsx:236`. `setState()` dentro de `useEffect`: `dashboard/authorization-inbox.tsx:83`, `point-of-sale/vin-selector-dialog.tsx:43`, `theme-toggle.tsx:11`. Mutación de ref en render: `components/pos/authorization/use-authorization-polling.ts:51`. Reasignación post-render: `reportes/comisiones/page.tsx:116`. Fix: mover `Date.now()` a `useEffect` o recibirlo como prop server-side; envolver side-effects detrás de eventos; acumular con `reduce` en vez de `forEach + let`.
  - **Warnings (12)** — `react-hook-form` `watch()` incompatible con RC en `cash-register/expense-dialog.tsx:191` (se elimina solo al reescribir en Fase 4). Variables no usadas repartidas en: `catalogo-client.tsx:44-45`, `recepcion-detail.tsx:548`, `reportes/caja/cash-report.tsx:141`, `workshop/[id]/deliver-modal.tsx:12` (imports), `workshop/[id]/page.tsx:24`, `api/auth-requests/pending/route.ts:12`, `api/comisiones/reglas/[id]/route.ts:154`, `discount-authorization-panel.tsx:27`. `prefer-const` en `api/inventory/receipts/route.ts:204`.
  - **`form.watch()` en modales de Caja → migrar a `useWatch({ control })`** — pendiente en `entrada-efectivo-dialog.tsx`, `expense-dialog.tsx`, `withdrawal-dialog.tsx`. `.watch()` dispara warning de React Compiler por falta de cleanup; `useWatch` es compiler-safe. Cerrar en batch de Fase 6 junto con el resto del lint-cleanup.
  - Snapshot crudo del output disponible via `npx eslint src/` en commit `453f9ce` (post-migración de Caja P5.6 ampliada).
