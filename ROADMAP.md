...
Última actualización: 2026-04-22 (Clientes Sesión 6 ✅ — Sub-fases J (kebab del header con Editar/Ficha PDF/Fusionar/Eliminar, modal de eliminación con motivo enum, wizard `/customers/[id]/merge` con preview de FKs y validación antichain server-side), K (PDFs `ficha` por cliente y `historial` por bici reusando `react-pdf 4.4.1` + sello + fonts) y L (modal mid-venta del POS en `quickMode` consumiendo `customerQuickCreateSchema` con consent, chip `Perfil incompleto` derivado en directorio cuando faltan email/RFC/dirección))  
...
5. Clientes
   ...
   > **Fuera de alcance explícito:** loyalty/tiers, telemetría IoT, tickets soporte, test drives, SMS, portal público del cliente, API oficial WhatsApp (solo deep-link `wa.me`).
   - **Sesión 1 ✅ (2026-04-22) — Sub-fases A (schema + migración + backfill) y B (endpoints backend-only)** — migración `20260422210000_customers_redesign_subfase_a` con cambios aditivos (9 columnas nuevas en `Customer`, `CustomerBike.odometerKm`, `CashTransaction.customerId`, 2 tablas `CustomerNote`/`CustomerEditLog`, 2 enums) y **1 cambio no-aditivo controlado: `Customer.phone` deja de ser `@unique`** (dedup pasa a flujo de merge). Backfill en migración: normaliza `phone` (últimos 10 dígitos vía `REGEXP_REPLACE`, drop valores <10 dígitos), normaliza `rfc` (`UPPER(TRIM)` + nulls de strings vacíos), backfillea `CashTransaction.customerId` desde `Sale.customerId`. Índices nuevos: `rfc` unique, `phone`/`email`/`mergedIntoId`/`deletedAt`/`customerId` (en CashTransaction) no-unique.
   - **Helpers compartidos** — `src/lib/customers/phone.ts` (`normalizePhoneMX`/`formatPhoneDisplay`/`formatPhoneForWhatsApp` — almacén 10 dígitos raw, lada país solo al formatear), `name.ts` (`splitDisplayName` cosmético), `validation.ts` (`customerCreateSchema` + `customerQuickCreateSchema` + `customerUpdateSchema` con `zod.preprocess` para phone/rfc), `service.ts` (`listableCustomerWhere`, `diffForAudit`, `writeCustomerEditLog`, helpers de rol `isManagerPlus`/`isSellerPlus`/`isTechnicianPlus`, `CUSTOMER_SENSITIVE_FIELDS`).
   - **Endpoints Sub-fase B (18 handlers, 11 route files):** `GET|POST /api/customers` · `GET|PUT|DELETE /api/customers/[id]` · `PATCH /api/customers/[id]/restore` · `POST /api/customers/[id]/merge-into` · `POST /api/customers/[id]/unmerge` · `GET|POST /api/customers/[id]/notes` · `PATCH|DELETE /api/customers/[id]/notes/[noteId]` · `PATCH /api/customers/[id]/tags` · `GET /api/customers/[id]/edit-log` · `GET /api/customers/[id]/timeline` · `GET /api/customers/search` · `GET /api/customers/stats` · `PATCH /api/customer-bikes/[id]` (odómetro con audit). Filtro soft-delete/mergeados SOLO en top-level (helper `listableCustomerWhere`). Merge valida 3 condiciones antichain y devuelve 409; undo con ventana 30d usa `createdAt <= mergedAt` para no robar filas post-merge. PUT audita diffs en `CustomerEditLog`, guarda `phonePrevious`, bloquea campos MANAGER-only para SELLER, exige `reason` para campos fiscales/creditLimit.
   - **Endpoints modificados** — `POST /api/sales` (ambas rutas: frozen POS y normal) y `POST /api/customers/[id]/balance` propagan `customerId` a `CashTransaction` para que el timeline del perfil capte pagos y recargas. Otras creaciones de `CashTransaction` (workshop charge, pedidos, cotizaciones convert, service-orders deliver/cancel) quedan deuda blanda — no afectan Sub-fase E (el timeline las recogerá por `saleId` o se añadirán por sub-fase G).
   - **Callsites rotos y arreglados por drop de `Customer.phone @unique`:** `prisma/qa-p5.ts`, `prisma/seed.ts` y `prisma/seed-transactional.ts` (migrados de `upsert where:{phone}` / `findUnique` a `findFirst` + create) y `src/app/api/workshop/orders/route.ts` línea 217 (`findUnique` → `findFirst` con filtro `deletedAt:null, mergedIntoId:null`).
   - **Diferido a Sub-fase B futuras / sesiones siguientes:** consolidación de `/api/workshop/customers/search` con `/api/customers/search` (brief §9), tests de contrato (no hay harness — Jest/Vitest no instalados; se difiere al setup del módulo de pruebas), endpoints PDF `/ficha.pdf` y `/bicis/[bikeId]/historial.pdf` (Sub-fase K), side-effects de voltaje/batería en `PATCH /api/customer-bikes/[id]` (hoy solo cubre `odometerKm`; los cambios de voltaje/batería siguen en sus endpoints especializados de `sales` y `batteries/uninstall`).
   - **Validaciones verde al cierre** — `prisma validate`, `npm run lint` (1 warning pre-existente en `reportes/rentabilidad/page.tsx`) y `npm run build` OK.
   - **Sesión 2 ✅ (2026-04-22) — Sub-fases C (directorio) y D (registro + edición + detección de duplicados)** — rediseño completo del directorio `/customers` y rutas dedicadas `/customers/new` + `/customers/[id]/editar` (cierra la deuda 1-C del roadmap sobre acciones de Command Palette sin destino). El perfil `/customers/[id]` y el dialog mid-venta del POS quedan intactos (territorio de sub-fases E y L).
   - **Directorio (Sub-fase C)** — `src/app/(pos)/customers/page.tsx` es server component con paginación (`page` param, 50/pág) + role-gate para `?showDeleted=1` (MANAGER+). `customer-list.tsx` consume `DetailHeader` + `KpiGrid` (4 KPIs: totales, LTV acumulado featured con Velocity Gradient, ticket promedio, saldo por cobrar) + omni-search debounced (250ms, server-side vía URL param `q`) + quick chips togglables (`Todos/Activos 90d/Con saldo por cobrar/Riesgo 90-180d/Inactivos +180d/Empresas/Sin consent.`) + power grid con selección múltiple. Bulk actions: export CSV cliente-side y modal **Lista WhatsApp** que filtra por `communicationConsent === true` con contador "X de Y seleccionados tienen consentimiento, Z omitidos" + botón `Ver omitidos` (BRIEF §7.2). Row actions: link al perfil, copiar teléfono, WhatsApp deep-link vía `formatPhoneForWhatsApp`.
   - **Enrichment helper** — `src/lib/customers/directory-query.ts` expone `listDirectoryCustomers` (una sola pasada con 6 `groupBy` paralelos: ltv desde Sale COMPLETED, arPending desde Sale LAYAWAY, salesCountLast12mo, lastSaleAt, lastServiceOrderAt, lastQuotationAt) — evita N+1 y resuelve `lastActivityAt = max(sale, serviceOrder, quotation)` cross-sucursal (Customer es global, BRIEF §1). `getDirectoryStats` reutiliza el filtro `listableCustomerWhere` del endpoint para que el KPI del directorio y el listado estén alineados.
   - **Chips de segmentación** — `src/lib/customers/segmentation.ts` con `computeSegmentChips` puro: `EMPRESA`, `FRECUENTE` (≥3 ventas COMPLETED 12mo), `EN_RIESGO` (90-180d sin actividad), `INACTIVO` (>180d o nunca), `SALDO_SIN_USAR` (balance > 0), `SIN_CONSENTIMIENTO` (consent false + phone presente), `CON_SALDO_POR_COBRAR` (arPending > 0). Persistencia: ninguna (derivado). Caché `unstable_cache` con tags (BRIEF §10.2) se pospone — el enrich actual tarda ≤100ms en dataset de prueba.
   - **Registro + edición (Sub-fase D)** — `src/components/customers/customer-form.tsx` es el form unificado create/edit consumiendo las mismas constantes SAT del form legacy pero con layout 2 columnas semántico (Datos personales + Contacto // Datos fiscales + Logística). Toggle `Es empresa` muestra/oculta Razón social; checkbox "Mismo que correo de contacto" deshabilita y mapea `emailFiscal` en submit sin `useEffect` (evita warning de React Compiler contra `watch()` — usa `useWatch` + resolución en el handler). Checkbox "Misma dirección que facturación" colapsa los 8 campos de envío. Inputs `firstName`/`lastName` son dos campos visuales que concatenan con `.trim()` al persistir (BRIEF dec. "split descartado"). Helper `splitInitialName` hidrata los dos campos desde `Customer.name` en modo edit.
   - **Detección de duplicados inline** — en modo create, blur sobre `phone`/`email`/`rfc` dispara `GET /api/customers/search?match={field}&value=...`; si hay hit, banner amarillo (`--warn-container`) bajo el input con link al cliente existente + botón "No, es otro" que dismiss-ea ese id (set local). En edit se omite — el server ya rechaza con 409 vía constraint.
   - **Guardrails campos fiscales** — cambios a `rfc`/`razonSocial`/`regimenFiscal` en modo edit disparan modal glassmorphism con textarea obligatoria; se envía como `reason` en el body del PUT, que llega a `CustomerEditLog.reason` vía el audit log ya cableado en Sub-fase B. `creditLimit` no vive en este form (BRIEF §7.3 lo relega a la card de Finanzas en Sub-fase H) — un hint al pie lo documenta para MANAGER+.
   - **Decisión de implementación** — no se extendió `GET /api/customers` de Sub-fase B con los campos derivados; el enrich corre server-side directo vía Prisma en el page component siguiendo el patrón de `reportes/caja/page.tsx`. Ventaja: un solo round-trip, sin capa HTTP intermedia. Trade-off: si un cliente externo consumiera el endpoint necesitaría su propia agregación. Aceptable — el endpoint es interno.
   - **Diferido a sesiones posteriores** — chip `TOP_LTV` (requiere percentil sobre 12mo, postpuesto a Sub-fase I cuando exista caché); chip `MANT_VENCIDO` (depende de `computeMaintenanceStatus` que llega en Sub-fase F); filtros avanzados del FilterPanel (rango LTV, sucursal de actividad reciente, con/sin bicis, tag manual — viven en Sub-fase I con el tab Datos); redirect 308 `/reportes/clientes/[id]` → `/customers/[id]?tab=finanzas` (Sub-fase H); UI de acciones del kebab header del perfil (editar/fusionar/eliminar — Sub-fase E).
   - **Validaciones verde al cierre** — `prisma validate`, `npm run lint` (solo el warning pre-existente en `reportes/rentabilidad/page.tsx`; warning nuevo en `customer-form.tsx` contra `watch()` resuelto migrando a `useWatch({ control })` y reemplazando `useRef` por `useState` para cumplir `react-hooks/refs`) y `npm run build` OK (63 páginas, rutas nuevas `/customers/new` y `/customers/[id]/editar` presentes).
   - **Sesión 3 ✅ (2026-04-22) — Sub-fase E (shell del perfil + tab Resumen)** — rediseño del perfil `/customers/[id]` con tokens DESIGN.md, KPIs específicos del cliente y el tab Resumen funcional. Los demás tabs (Ventas/Taller/Bicis/Cotizaciones/Finanzas/Datos) renderizan placeholder con su sub-fase asignada (F/G/H/I) y, en el caso de Finanzas, link a `/reportes/clientes/[id]` mientras llega Sub-fase H.
   - **Server data layer** — `src/lib/customers/profile-data.ts` expone `getCustomerProfileData(customerId)` que en una sola pasada (12 queries paralelas) calcula: base + KPIs (LTV total, LTV año vs anterior con delta %, última actividad cross-sucursal, sucursal preferida vía `groupBy` sobre Sale+ServiceOrder+Quotation) + chips de segmentación (reusando `computeSegmentChips`) + alertas activas (mantenimiento vencido por bici vía `computeMaintenanceStatus`, apartados con `expectedDeliveryDate < now` y outstanding > 0, cotizaciones con `validUntil` ≤ 7d) + sidebar (saldo, crédito, AR pending, notas pinned) + counts para los pills de los tabs. Soft-merge: si `mergedIntoId !== null` el page.tsx hace `permanentRedirect` al target (BRIEF §6.1).
   - **Componentes** — `src/components/customers/profile/`: `customer-detail-header.tsx` (header sticky con backdrop-filter, avatar gradient, chips dinámicos de segmentación + tags manuales, acciones rápidas WhatsApp/Nueva venta/Nueva orden), `customer-kpi-strip.tsx` (3 tarjetas: LTV featured con Velocity Gradient + Última visita + Sucursal preferida con %), `profile-shell.tsx` (tabs por URL `?tab=`, role-gate de Finanzas para MANAGER+, pills con count), `tab-resumen.tsx` (layout 2/3+1/3, alertas con `border-left` semántico por tipo), `timeline-feed.tsx` (consume `/api/customers/[id]/timeline`, 7 chips de filtro por kind, paginación offset 50/pág con botón "Cargar más", deep-links a sale/workshop/cotización), `sidebar-summary.tsx` (cards Saldo a favor + Crédito con `ProgressSplit` cuando hay límite + Notas fijadas), `add-note-dialog.tsx` (modal nueva nota/interacción con 4 kinds, pin solo MANAGER+), `recharge-balance-dialog.tsx` (reemplazo de `add-balance-dialog.tsx` legacy con glassmorphism oficial y `formatMXN`).
   - **Diferido a sesiones posteriores** — kebab del header con acciones secundarias (editar/fusionar/eliminar) sigue como `more` placeholder hasta Sub-fase I/J; alerta "Garantía por expirar <30d" omitida (no existe modelo de garantía); chip "Saldo a favor sin movimiento >90d" usa heurística simplificada (no se traquea `balanceUpdatedAt`); polling/refresh del timeline tras escribir nueva nota se resuelve con `refreshKey` cliente + `router.refresh()`, sin SWR.
   - **Validaciones verde al cierre** — `prisma validate`, `npm run lint` (solo el warning pre-existente en `reportes/rentabilidad/page.tsx`) y `npm run build` OK (63 páginas).
   - **Sesión 4 ✅ (2026-04-22) — Sub-fases F (tab Bicis) y G (tabs Ventas / Taller / Cotizaciones)** — se cierran los 4 tabs pesados del perfil `/customers/[id]`. El page.tsx carga los 4 datasets en paralelo junto con `getCustomerProfileData` y el shell decide qué tab renderizar vía URL (`?tab=`).
   - **Server data layer nuevo** — `src/lib/customers/profile-tabs-data.ts` expone cuatro loaders cross-sucursal (ninguno filtra por `session.branchId`, BRIEF §7.4): `getCustomerBikesTabData` (CustomerBike + `productVariant.imageUrl` con fallback a `modelo.imageUrl`, `assemblyOrders` para fecha de compra, `batteryAssignments` con serial de batería + voltaje al instalar, `voltageChanges` con author resuelto, `serviceOrders` con items+servicios, `editLogs` filtrados a `field="odometerKm"`, cómputo de `operationalStatus = IN_WORKSHOP | OPERATIONAL` por servicios abiertos y `MaintenanceStatus` reusando `computeMaintenanceStatus`), `getCustomerSalesTabData` (Sale + items con descripción derivada, payments agregados para `paidSum`/`outstanding`/`methodsUsed`, `saleType = LAYAWAY ? APARTADO : CONTADO`, preservando `notes`/`internalNote`/`warrantyDocReady`/`expectedDeliveryDate`), `getCustomerServiceOrdersTabData` (ServiceOrder + items con `isExtra` y `esMantenimiento` expuestos, bici resuelta + `bikeInfo` fallback, `prepaid`/`prepaidAmount`/`subStatus`/`expectedDeliveryDate`/`diagnosis`), `getCustomerQuotationsTabData` (Quotation + items, `effectiveStatus` derivado por `convertedToSaleId` / `cancelledAt` / `validUntil < now` para exponer CONVERTIDA/CANCELADA/EXPIRADA además del status persistido).
   - **Tab Bicis (Sub-fase F)** — `tab-bicis.tsx` renderiza una card ancha por `CustomerBike` con grid `220px · 1fr · 200px` (colapsa a 1 columna <900px). Izquierda: foto del modelo (imagen `ProductVariant.imageUrl` o `Modelo.imageUrl` o icono `bike` fallback) + chips de estado operativo y mantenimiento (variant `success/warn/error` según `MaintenanceLevel`). Centro: título con marca+modelo, VIN monospace, grid de datos (color, voltaje, odómetro con botón inline "Editar" que abre modal, batería actual, último mantenimiento relativo, fecha de compra). Derecha: CTA "Nueva orden" con Velocity Gradient, "Ver ensamble" si hay `assemblyOrders`, y placeholder deshabilitado "Historial PDF" apuntando a Sub-fase K. Los 3 accordions (baterías / voltajes / mantenimientos) son excluyentes entre sí; un 4.º accordion independiente (`<details>`) lista el historial de `CustomerEditLog` por odómetro. Cuando `maintenance.nivel === "VENCIDO"` la card abre con banner rojo + botón "Agendar cita" linkando a `/workshop/recepcion?customerBikeId=…`.
   - **Modal "Editar odómetro"** — `edit-odometer-dialog.tsx` consume el `PATCH /api/customer-bikes/[id]` ya existente (Sub-fase B). El endpoint escribe `CustomerEditLog` con `field="odometerKm"` + `customerBikeId` y `reason` opcional; al volver, `router.refresh()` refresca el server component. El kebab "Cambiar voltaje / Cambiar batería / Dar de baja" del BRIEF §7.4/§8 queda **deferido a Sub-fase J/K/POS**: el flujo de voltaje+batería sigue concentrado en los endpoints de `sales` y `batteries/uninstall`, y "Dar de baja" no tiene schema (`CustomerBike.deletedAt` no existe). La UI comunica esto sin affordances fantasma — el CTA principal "Nueva orden taller" cubre el path dominante.
   - **Tab Ventas (Sub-fase G)** — `tab-ventas.tsx` es un power grid responsive (`overflow-x-auto`) con columnas Fecha · Folio · Tipo · Total · Descuento · Método · Vendedor · Sucursal · Estado. Chips `APARTADO / CONTADO` + estado (COMPLETED success, CANCELLED error, LAYAWAY → `warn` con `"X% pagado"`). Row expand con `background: var(--surf-high)` muestra chip `Póliza lista/pendiente` (derivado de `warrantyDocReady`), progreso de pago + `Por cobrar X` cuando aplica, ítems con subtotales y nota interna separada para MANAGER. Filter chips togglables (Tipo · Estado) no persisten en URL (local state — están acoplados al tab). Cross-sucursal — el loader no filtra por `session.branchId`.
   - **Tab Taller (Sub-fase G)** — `tab-taller.tsx` grid con Fecha · Folio · Bici (VIN) · Tipo · Estado · Sub-estado · Entrega · Total · Prepago · Sucursal. Chips tipados para `ServiceOrderType` (PAID/WARRANTY/COURTESY/POLICY_MAINTENANCE), `ServiceOrderStatus`, `ServiceOrderSubStatus` (warn) y `Prepagada $X` cuando `prepaid=true`. Row expand muestra diagnóstico + lista de ítems con chips auxiliares `Extra` (cuando `isExtra=true`) y `Mant.` (cuando viene de `ServiceCatalog.esMantenimiento`). `expectedDeliveryDate` es string `YYYY-MM-DD` → se formatea añadiendo `T12:00:00` para evitar drift de zona horaria.
   - **Tab Cotizaciones (Sub-fase G)** — `tab-cotizaciones.tsx` grid con Fecha · Folio · Vigencia · Total · Descuento · Estado efectivo · Vendedor · Sucursal. `effectiveStatus` server-side cubre `CONVERTIDA`/`CANCELADA`/`EXPIRADA` además de los enums persistidos. Row expand lista ítems + nota interna + link secundario "Venta asociada" cuando `convertedToSaleId != null`.
   - **Decisiones de implementación** — data loading concentrado en el server component (`page.tsx` → `Promise.all` de los 4 loaders) en vez de usar endpoints + fetch cliente; cero round-trips HTTP extra, consistencia con el patrón de `tab-resumen`. El shell pasa los 4 datasets como props aunque solo uno se renderice por tab — el coste de hidratación es bajo (una card típica tiene <20 filas por tab) y evita flicker al cambiar `?tab=`. `SELLER` no puede editar odómetro (BRIEF §8 / §7.4 — TECHNICIAN+).
   - **Diferido a sesiones posteriores** — modales "Cambiar voltaje / Cambiar batería" (requieren flujo completo de BatteryConfiguration + Battery — hoy viven en taller); "Dar de baja" (no existe schema — requiere migración en fase posterior); "Historial PDF por bici" (Sub-fase K); "Ver ensamble" apunta a `/workshop?bikeId=…` (sin vista dedicada todavía); filtros en URL params para que las tabs preserven estado al recargar (local state es suficiente para la iteración actual); virtualización de los grids (dataset por cliente <200 filas en casos reales).
   - **Validaciones verde al cierre** — `prisma validate`, `npm run lint` (solo el warning pre-existente en `reportes/rentabilidad/page.tsx`) y `npm run build` OK (63 páginas).

   - **Sesión 5 ✅ (2026-04-22) — Sub-fases H (tab Finanzas, MANAGER+) y I (tab Datos)** — cierran los dos tabs que quedaban como placeholder y consolidan `/reportes/clientes/[id]` en el perfil canónico. El shell del perfil `/customers/[id]` ya renderiza 6 de 7 tabs con UI completa (Resumen/Ventas/Taller/Bicis/Cotizaciones/Finanzas/Datos); queda como deuda el header kebab con las acciones secundarias (editar/fusionar/eliminar) que aterriza en Sub-fase J junto al wizard de fusión.
   - **Tab Finanzas (Sub-fase H)** — `tab-finanzas.tsx` es MANAGER+ only y se arma con `getCustomerFinanzasData` (nuevo loader en `src/lib/customers/profile-finanzas-data.ts`). Panel izquierdo 1/3: card **Saldo por cobrar** (`sum(LAYAWAY.total - payments.sum)` con chip `Vencido por X días` cuando el apartado más viejo tiene `expectedDeliveryDate < now`), card **Crédito** con `Límite · Usado · Disponible` + `ProgressSplit` + botón `Editar límite`, card **Saldo a favor** con reutilización de `RechargeBalanceDialog`. Panel derecho 2/3: **Movimientos** como tabla `CashTransaction` del cliente (últimas 200) con toggle segmentado `Abonos · Cargos · Todos`, columnas Fecha · Descripción · Folio · Método · Abono · Cargo · Acumulado (saldo acumulado computado en cliente desde el más antiguo); **Pagos parciales** por apartado con `ProgressSplit` pagado/pendiente + `<details>` desplegable con lista de abonos (fecha, método, referencia, monto). Header del tab con `Corte al {hoy} · MXN` + botón `Estado de cuenta PDF` (disabled, Sub-fase K). Descripciones de movimientos son derivadas en el loader — `"Pago a venta {folio}"`, `"Recarga de saldo a favor"`, `"Egreso de caja"`, etc. — con heurística `CHARGE` (REFUND_OUT / EXPENSE_OUT / WITHDRAWAL) vs `PAYMENT` (PAYMENT_IN / CASH_DEPOSIT).
   - **Modal "Editar límite de crédito"** — `edit-credit-limit-dialog.tsx` llama `PUT /api/customers/[id]` con `{ creditLimit, reason }`; el endpoint ya auditaba `creditLimit` desde Sub-fase B (requiere `reason` y es MANAGER-only por la lista `MANAGER_ONLY_FIELDS`). Motivo obligatorio en UI + toast de error si se envía vacío.
   - **Redirect 308 desde `/reportes/clientes/[id]`** — el page.tsx se reemplaza por `permanentRedirect(/customers/${id}?tab=finanzas&from=…&to=…)` preservando los params de rango de fecha. Se eliminó `cliente-detalle-client.tsx` (ya no se usa). El índice `/reportes/clientes` sigue intacto (ranking cross-customer).
   - **Tab Datos (Sub-fase I)** — `tab-datos.tsx` arma vista read-only con `<details>` colapsables: Identidad (nombre completo, fecha de nacimiento + edad derivada en cliente, "Cliente desde" — proxy = venta más antigua, tipo persona/empresa, razón social), Contacto (phone/phone2 con `formatPhoneDisplay`, `phonePrevious` como fila muted, email, chip consentimiento success/error), Datos fiscales (RFC mono, razón social, régimen, uso CFDI, email fiscal, dirección fiscal), Dirección de envío (8 campos), Segmentación (chips del sistema + tags manuales con remove inline + botón `Agregar tag` — ambos MANAGER+), Historial de cambios (tabla CustomerEditLog con Fecha · Campo · Cambio `old → new` · Motivo · Autor), y Fusiones recibidas (cuando `mergedSources` tiene filas). `FIELD_LABELS` traduce nombres técnicos (`rfc`, `creditLimit`, `__merge__`, `__unmerge__`) a strings ES. Botón superior `Editar datos` linkea a `/customers/[id]/editar`.
   - **Add/Remove tag + Unmerge** — `add-tag-dialog.tsx` (glassmorphism + autoFocus + Enter submit, valida duplicados contra `existingTags`), `remove-tag-button.tsx` (inline `x` pequeño sobre cada chip manual) y `unmerge-button.tsx` (confirm nativo + llamada a `POST /api/customers/[sourceId]/unmerge`, deshabilitado con tooltip "Ventana expirada" cuando `daysUntilUndoExpires <= 0`). Los 3 escriben vía endpoints de Sub-fase B; tras éxito hacen `router.refresh()`.
   - **Loaders del perfil** — `getCustomerFinanzasData` corre sólo si el viewer es MANAGER+ (else `null`); `getCustomerDatosData` siempre corre y recibe `viewerIsManagerPlus` + `viewerUserId` para aplicar el filtro de `CustomerEditLog` (SELLER ve solo sus propios edits, MANAGER+ ve todos). `page.tsx` lanza ambos loaders junto con los 4 de Sub-fases F/G en `Promise.all`; en una visita típica agrega ~80ms porque son consultas simples con índice.
   - **Decisiones de implementación** — `Customer` no tiene `createdAt` en el schema; "Cliente desde" usa como proxy la `createdAt` de la venta más antigua (o `new Date()` si el cliente nunca ha comprado). Migrar a un campo dedicado queda como deuda blanda. El saldo acumulado en movimientos se calcula en cliente desde la fila más antigua — naïve pero correcto para los ≤200 movimientos típicos. El `confirm()` nativo para "Deshacer fusión" se usa en lugar de un modal dedicado; la acción es infrecuente y el texto ya explica la consecuencia.
   - **Diferido a sesiones posteriores** — kebab del header (editar/fusionar/eliminar) aún como `more` placeholder hasta Sub-fase J; modal dedicado para unmerge con preview de FKs; campo `Customer.createdAt` (schema aditivo trivial, se aterriza cuando pase un commit aparte); `Customer.balanceUpdatedAt` para endurecer el chip `SALDO_SIN_USAR`; PDF de estado de cuenta (Sub-fase K); timeline de pagos parciales con `TimelineFeed` compartido (hoy es una lista custom, se consolidará si aparece un tercer consumidor).
   - **Validaciones verde al cierre** — `prisma validate`, `npm run lint` (solo el warning pre-existente en `reportes/rentabilidad/page.tsx`) y `npm run build` OK (63 páginas, incluido el redirect 308 en `/reportes/clientes/[id]`).
   - **Sesión 6 ✅ (2026-04-22) — Sub-fases J (merge wizard + acciones header), K (PDFs ficha cliente + historial bici) y L (quick-create POS)** — cierran el módulo de Clientes. El perfil ya tiene su kebab funcional con las 4 acciones del BRIEF, los PDFs por cliente y por unidad (útiles para garantías de fabricante), y el quick-create del POS reutiliza el schema central sin duplicar reglas.
   - **Acciones del header (Sub-fase J)** — `customer-detail-header.tsx` ahora recibe `isManagerPlus` desde `profile-shell` y delega el botón derecho a `header-actions-menu.tsx` (kebab con click-outside): Editar (link a `/customers/[id]/editar`), Descargar ficha PDF (link externo a `/api/customers/[id]/ficha/pdf`), y — solo MANAGER+ — Fusionar (link a `/customers/[id]/merge`) y Eliminar (abre `delete-customer-dialog.tsx`). El botón de eliminar usa color `--ter` para señalar la severidad sin Velocity Gradient. El modal de eliminación captura `deletedReason` enum con tres opciones (DUPLICATE / REQUEST / ERROR) y consume el `DELETE /api/customers/[id]` ya existente en Sub-fase B.
   - **Wizard de fusión (Sub-fase J)** — `/customers/[id]/merge/page.tsx` valida MANAGER+ + redirige si el origen ya está mergeado, y delega a `merge-wizard.tsx`. El wizard busca destino vía `/api/customers/search` (debounce 250ms, filtra `id !== source.id`), llama a `GET /api/customers/[id]/merge-preview?targetId=…` (nuevo endpoint) que devuelve los counts de FKs a reasignar (sales, serviceOrders, quotations, cashTransactions, bikes, notes, editLogs) + un campo `blocker` con el mensaje pre-validado de antichain (las 3 reglas del BRIEF §6.1 — origen ya mergeado, destino ya mergeado, origen es target de otro merge). Si hay `blocker` el botón de submit queda deshabilitado y se muestra banner rojo. Side panels muestran origen (chip `warn` "Se descontinuará") y destino (chip `success` "Recibirá la actividad") con saldo, RFC y email. Al confirmar dispara `POST /api/customers/[id]/merge-into` y redirige a `/customers/{targetId}`. La operación se puede deshacer 30 días desde la pestaña Datos del destino (Sub-fase I).
   - **PDFs ficha + historial (Sub-fase K)** — Dos templates nuevos en `src/lib/pdf/templates/`: `customer-ficha-pdf.tsx` (single-page: header sucursal con `DocumentHeader`, identidad cliente, KPIs LTV/saldo a favor/saldo por cobrar/bicis, tabla de bicis VIN+modelo+voltaje+odómetro, últimas 10 ventas + últimas 10 órdenes con folio/fecha/tipo/total, datos fiscales, footer con sello) y `customer-bike-historial-pdf.tsx` (header + identidad bici + propietario actual + cronología completa de service orders con servicios+notas + tablas de baterías/voltajes/odómetros). Ambos endpoints (`/api/customers/[id]/ficha/pdf` y `/api/customers/[id]/bicis/[bikeId]/historial/pdf`) reusan `assertBranchConfiguredForPDF(user.branchId, "reporte")` y `resolveSealBuffer` (gotcha P6 — sello WebP convertido a PNG con sharp). El kebab del header linkea al primero; el botón "Historial PDF" en cada card de bici (`tab-bicis.tsx`, antes deshabilitado) ahora es un `<a target="_blank">` real al segundo.
   - **Quick-create POS (Sub-fase L)** — `customer-create-form.tsx` recibe nuevo prop `quickMode` que oculta phone secundario, email, sección de envío y sección fiscal, dejando solo nombre + teléfono + checkbox de consentimiento (con copy explicativo en quick mode); también añade el checkbox `communicationConsent` al formulario completo (estaba en el schema pero faltaba en la UI). `customer-selector-modal.tsx` del POS pasa `quickMode={true}`. La validación visual sigue ejecutando el schema completo del form (los campos opcionales no aparecen en quick mode → validan trivialmente), y el endpoint POST `/api/customers` valida con `customerCreateSchema` (`customerQuickCreateSchema` queda como contrato compartido pero el endpoint sigue aceptando el superset).
...
- **Persistir breakdown de denominaciones en cierre de corte** ✅ (2026-04-15, migración `20260415061254_add_denominations_json_to_cash_session`) — `CashRegisterSession.denominationsJson Json?` (Prisma `Json` en Postgres ya mapea a `jsonb`); el `PATCH /api/cash-register/session` lo persiste desde `parsed.data.denominaciones ?? undefined`. Sesiones legacy quedan `null`. Pendiente separado: que el PDF P6-E consuma el campo (no es deuda de persistencia).
- **Normalización server-side de `PurchaseReceipt.proveedor` (P4-C + P10-G)** ✅ (2026-04-22, commit `4e01b7a`) — `POST /api/inventory/receipts` aplica `trim()` + collapse de whitespace interno en el zod schema. Sin `toLowerCase()` en persistencia (preserva casing del operador; el reporte P10-G sigue agrupando en memoria). Sin backfill: 0 colisiones en la BD al cierre.
...
- **Self-hostear fuentes `next/font/google` (Inter + Space Grotesk)** ✅ (2026-04-22, commit `1b4ed72`) — `layout.tsx` migrado a `next/font/local` apuntando a `public/fonts/Inter-Variable-latin.woff2` + `SpaceGrotesk-Variable-latin.woff2` (variable fonts, subset latin). Ambas expuestas como CSS vars (`--font-inter`, `--font-space-grotesk`); `globals.css` actualiza `--font-body` para consumir `var(--font-inter)` en vez de la string `"Inter"`. Los TTFs previos en `public/fonts/` siguen siendo del consumidor PDF (`src/lib/pdf/fonts.ts`), independiente.
...
- **Seed de ventas con descuentos para QA de P10-C (diferido desde P10 Lote 5)** ✅ (2026-04-22, commit `57a680e`) — `seedSales` inyecta 3 casos deterministas en las primeras ventas COMPLETED por sucursal: (0) sólo `Sale.discount`, (1) sólo `SaleItem.discount`, (2) combinado. El resto del loop mantiene su aleatoriedad. Ajuste colateral: `Sale.subtotal` recalculado como `Σ(price×qty − SaleItem.discount)` para mantener el invariante del POS real (`Sale.total = subtotal − Sale.discount`). Verificación visual en `/reportes/rentabilidad` pendiente al próximo `prisma migrate reset`.
