...
**Etapa: desarrollo / pre-producción.** Aún no hay usuarios reales operando el sistema, no hay datos de negocio en producción, y no hay obligación de uptime o backwards-compatibility con data viva. Esto significa:

- Migraciones destructivas (DROP columna, ALTER NOT NULL, rename de modelo, etc.) son aceptables sin estrategia de zero-downtime ni feature flags. No hace falta dual-write largo ni shadow-write con drift detectors a menos que ya se esté implementando uno por otra razón.
- `prisma migrate reset` es válido en local cuando un drift es más caro de reconciliar que de tirar — siempre y cuando el seed regenere el módulo afectado QA-able sin pasos manuales (ver convención seed outcome-based en memory).
- Los CSVs de migración masiva del cliente (inventario, clientes, proveedores, recibos legacy) **aún no han llegado**. Hasta que lleguen, cualquier "cutover" contra strings legacy puede diferirse y consolidarse en una sola pasada cuando exista la data real.
- "Pack X bloqueado por cliente" en el roadmap normalmente quiere decir "esperando decisión de negocio o data legacy", no "deploy bloqueado".

Cuando el proyecto pase a producción real, esta sección se reescribe — y a partir de ese momento aplican freezes, migraciones additive-only por defecto, y los cutovers requieren estrategia de zero-downtime.
...
- **Velocity Gradient**: máximo 3 instancias por vista (KPI destacado + CTA primario + elemento del shell). Excepciones adicionales permitidas: toggles de rango temporal y barras de progreso comparativo.
...
- **Conteo de Velocity Gradient en el shell**: el chip BRANCH (non-admin) y el avatar del footer del sidebar consumen 1-2 instancias del cupo de 3 permitidas por vista. Las vistas hijas tienen cupo restante para KPI destacado + CTA primario sin exceder el límite. Resolución registrada 2026-04-19 en Sesión 4 V1 Ventas e ingresos.
...
- **Command Palette global** (1-C, 2026-04-17): trigger centrado en topbar (`src/app/(pos)/search-trigger.tsx`) + palette en `src/app/(pos)/command-palette.tsx` montado fuera de `<main>` (al lado de `CashSessionManager`). Apertura via `window.dispatchEvent(new CustomEvent('command-palette:open'))` — no hay provider/contexto. Atajo global `⌘K` (macOS) / `Ctrl K` resuelto con `useSyncExternalStore(...navigator.platform.includes('Mac'))` (SSR-safe, cero hydration mismatch). El palette es **navegación + búsqueda**, nunca ejecuta mutaciones — las "acciones" del default state son `router.push` a formularios existentes (`/point-of-sale`, `/cotizaciones/nueva`, `/inventario/recepciones/nuevo`, `/cash-register`). Navegación filtrada por `session.user.role`; `AuthorizationRequest` solo visible a MANAGER/ADMIN.
- **Endpoint `GET /api/search`** (`src/app/api/search/route.ts`): búsqueda unificada en 8 entidades (Customer, ServiceOrder, Sale, Producto, Quotation, PurchaseReceipt, OperationalExpense, AuthorizationRequest). Patrón **híbrido** obligatorio: `$queryRaw` con `LOWER(unaccent(col))` para matching + ranking + `COUNT(*) OVER()` (el IDs con `score 0=exact, 1=starts-with, 2=contains` + count total); luego hidratación con Prisma ORM (`findMany({ where: { id: { in: ids } } })`) type-safe. Nunca `$queryRawUnsafe` con input del cliente. Filtro de branch se construye con un helper `branchFilter(branchId: string | null, alias?: string): Prisma.Sql` que devuelve `Prisma.empty` para ADMIN (`branchId === null`) o `AND <alias>."branchId" = ${branchId}` para el resto. `Customer` y los productos son globales (sin `branchId`). `AuthorizationRequest` se filtra adicionalmente por `status IN ('PENDING','APPROVED','REJECTED')` y `createdAt >= NOW() - INTERVAL '30 days'`. Tablas raw que NO coinciden con el nombre del modelo: `"ModeloConfiguracion"` (ProductVariant via `@@map`), `"cotizaciones"` (Quotation via `@@map`). Todo el payload tipa con `SearchCategory`/`SearchResult`/`SearchGroup`/`SearchResponse` exportados — el cliente importa directo desde la ruta del route handler (`import type { SearchResponse } from "@/app/api/search/route"`), mismo patrón que `NotificationFeedResponse`.
- **Extensión `unaccent`** (migración `20260418015524_enable_unaccent_extension`): habilitada globalmente (`CREATE EXTENSION IF NOT EXISTS unaccent;`). Prisma no la maneja declarativamente — la migración se creó con `--create-only` y el SQL se editó a mano. Usar `LOWER(unaccent(columna))` para cualquier búsqueda acento-tolerante en raw SQL.
...
- **ExportDrawer (Sesión 5 — 2026-04-19)** — Endpoint universal `POST /api/reportes/[slug]/export` con registry en `src/lib/reportes/export-registry.ts`. Tipos en `src/lib/reportes/export-types.ts`. Generadores en `src/lib/reportes/generators/` (`csv.ts`, `xlsx.ts`, `pdf.tsx`). Handler V1 en `src/app/(pos)/reportes/ventas-e-ingresos/export-handler.ts`. UI en `src/components/reportes/shell/export-drawer.tsx`. Cap síncrono de 10,000 filas — responde 413 si excede. Semáforo en-memory `Map<userId, Promise>` evita exports paralelos del mismo usuario (429). Librerías: `exceljs@4.4.0` (nuevo), `@react-pdf/renderer@4.4.1` (existente). Formatos: CSV (UTF-8 BOM, `\r\n`), XLSX (header freeze row 4, fill verde #1B4332), PDF (A4 portrait/landscape según nº columnas, headers fijos por página). Para añadir un nuevo reporte exportable: crear un `ExportHandler` en el directorio del reporte y registrarlo en `EXPORT_HANDLERS` del registry. `getSalesTableForExport` en `queries.ts` de ventas usa `take: 10_001` para detección de saturación en un solo query.
- **Reportes expandidos infraestructura (P10 Lote 1 — 2026-04-16)** — Capa compartida en `src/lib/reportes/` (6 módulos) y `src/app/(pos)/reportes/_components/` (6 componentes). **`downloadCSV`** recibe `Record<string, unknown>[]` — las claves del primer objeto son los headers; NO usar la API antigua de `(headers[], rows[][], filename)`. **Enums de Prisma en clientes CSV**: igual que en el POS, los enums son server-only; definir tuplas locales `as const`. **`branchWhere(session, filterBranchId?)`**: retorna `{}` para ADMIN sin filtro, `{ branchId: filterBranchId }` para ADMIN con filtro, `{ branchId: session.branchId }` para MANAGER, `{ branchId: "__none__" }` si MANAGER sin branchId. **`parseDateRange`**: el `to` se ajusta a `23:59:59.999` del día — al mostrar en UI restar 1ms con `new Date(toDate.getTime() − 1)` antes de `toDateString()`. **Filtro cross-model en stock**: `quantity ≤ productVariant.stockMinimo` no es expresable en Prisma `where` — fetch con `stockMinimo > 0` y filtrar in-memory. **Sidebar `hasSpecificSibling`**: cuando una ruta padre (`/reportes/caja`) y una ruta hija específica (`/reportes/caja/historial`) coexisten como sub-items, el padre usa match exacto (`pathname === child.href`) para evitar que quede activo en la ruta hija. **TZ filtros de fecha**: cerrado en Pre-Fase 6 Paso 1 (2026-04-17) — los 7 archivos listados migraron a `parseLocalDate(param, endOfDay)`.
- **Reportes P10 Lote 6 (P10-G — 2026-04-17)** — Reporte agregado de compras al proveedor en `/reportes/compras-proveedor`. **Normalización en memoria**: `proveedor.trim().toLowerCase()` como key de agrupación; nombre canónico = el más frecuente del grupo (desempate alfabético). NO toca la BD — la normalización server-side en `POST /api/inventory/receipts` es deuda Fase 6. **⚠️ Deuda normalización**: la BD QA usa seeds consistentes, por lo que los casos de variación de case/espacios no se ejercitaron. Al cargar datos reales, verificar `/reportes/compras-proveedor` — si aparecen grupos duplicados, la normalización en `POST /api/inventory/receipts` pasa a alta prioridad. **Timezone**: cerrado en Pre-Fase 6 Paso 1.5 (2026-04-17) — `fechaVencimiento` pasó a `String?` "YYYY-MM-DD"; comparaciones ahora son lex contra `toDateString(new Date())`. Ver §Decisiones clave.
- **Reportes P10 Lote 5 (P10-C/D — 2026-04-16)** — **`resolveCostsBatch(variantIds, simpleIds)`** (`src/lib/reportes/cost-resolver.ts`): resolución global (sin filtro de `branchId`), 3 queries — (1) `InventoryMovement(PURCHASE_RECEIPT, precioUnitarioPagado NOT NULL)` desc, primera aparición por key = más reciente; (2) `ProductVariant.costo` / `SimpleProduct.costoInterno` para IDs sin RECEIPT (paralelo); (3) fallback `{ cost: 0, source: "NONE" }` en memoria. Keys del Map: `"v:{variantId}"` / `"s:{simpleId}"`. **`computeLineRevenues(sale)`** (`src/lib/reportes/line-revenue.ts`): prorrateo de `Sale.discount` proporcional al ingreso bruto de cada línea. `IVA_RATE = 0.16` exportado de `money.ts`. Fórmula: `revenueNeto = (price × qty − lineDiscount − prorated_globalDiscount) / 1.16`. **Convención de pricing** (confirmada en P5-E + P6-C y cerrada en Lote 5): `SaleItem.price` = precio de lista con IVA; `SaleItem.discount` = monto total de descuento de la línea en pesos; `Sale.discount` = descuento global adicional en pesos a prorratear. **Diagnóstico de descuentos** (`prisma/diagnostic-p10c-discount.ts`): BD QA tiene 0 ventas con descuento — el prorrateo es neutral. Ejecutar con `npx tsx prisma/diagnostic-p10c-discount.ts`. **⚠️ Deuda Fase 6:** (1) `resolveCostsBatch` descarta duplicados en memoria — optimizar con `DISTINCT ON` cuando el volumen lo justifique; (2) seed de ventas con `Sale.discount > 0` y `SaleItem.discount > 0` para ejercitar `computeLineRevenues` en QA (ver ROADMAP.md §Fase 6).
...
- **Transferencias cross-branch (Fase P12-B — 2026-04-17)** — Flujo completo de `StockTransfer` (SOLICITADA → BORRADOR → EN_TRANSITO → RECIBIDA | CANCELADA). **Patrones de UI:** `TransferStatusBadge` vive en `src/components/` (componente compartido sin lógica de negocio); constantes de estilo de módulo centralizadas en `transferencias/shared-tokens.ts` (evita copiar MODAL_STYLE/INPUT_STYLE en cada dialog). **Quick-action sin duplicar modales:** los botones de acción rápida en la tabla navegan a `detail?modal=ACTION`; el detail client abre el dialog correcto via lazy `useState(() => autoModal ?? null)` — no se duplica UI entre lista y detalle. **`fetchedFor` para loading derivado:** cuando un effect hace fetch condicionado a un branchId, guardar `fetchedFor: string|null` y derivar `loading = !!branchId && fetchedFor !== branchId`; evita el `setState` síncrono dentro del effect que dispara `react-hooks/set-state-in-effect`. **Resets en event handler, no en effect:** `useEffect([open])` que resetea estado de formulario → mover toda la lógica de reset al `handleOpenChange` wrapper (React Compiler bloquea setState síncrono en effects). **Endpoint polimórfico de items disponibles:** `GET /api/transferencias/items-disponibles?branchId=X` retorna las 4 categorías (variantes con stock>0, simpleProducts con stock>0, baterías IN_STOCK, customerBikes sin cliente) en un solo request — no construir 4 endpoints separados cuando el consumer siempre necesita los cuatro. **`CustomerBike.brand` y `.model` son `String?` (nullable)** en el schema Prisma — siempre usar `[cb.brand, cb.model].filter(Boolean).join(" ")` al construir labels; nunca asumir non-null. **Role-scoped branch selectors en NuevaTransferenciaDialog:** SELLER (fromBranch = otras sucursales, toBranch = propia readonly); MANAGER (fromBranch = propia readonly, toBranch = selector); ADMIN (ambos selector libre). La lógica de scoping en la UI replica exactamente los `where` clauses del Server Component y del API.

- **Reportes de transferencias y mermas (Fase P12-C — 2026-04-17)** — Dos reportes server-only en `/reportes/transferencias` y `/reportes/transferencias/mermas` (MANAGER+ADMIN). **Branch scoping con OR manual**: `branchWhere()` de `branch-scope.ts` no aplica a `StockTransfer` porque el modelo tiene dos campos de sucursal (`fromBranchId`/`toBranchId`); para MANAGER se construye `where.OR = [{ fromBranchId }, { toBranchId }]` directamente, igual que en `/transferencias/page.tsx`. **KPIs separados del `where` filtrado**: los contadores (enTransito/recibidas/canceladas) usan `kpiBase` sin los filtros de status/folio/branches del usuario — el total del rango no se ve afectado por los filtros de exploración. **Agrupación client-side para mermas**: todos los `MermaRow` llegan del servidor sin paginar (subconjunto pequeño por definición); las vistas "por producto" y "por sucursal" se derivan en el cliente con `Map` + `Set<transferId>` para deduplicar transferencias. Drill-down via `useState` (sin URL params polimórficos, sin round-trip). **CSV siempre detalle completo** ignorando `agruparPor` — las vistas agrupadas son UI, el CSV es fuente de verdad granular. **Helper compartido** `src/lib/reportes/transferencias.ts`: `formatProducto(item)` y `computeMermaUnidades(enviada, recibida)` reutilizados en ambos reportes. **`ReportDateFilter` usa `from`/`to`** (no `desde`/`hasta`) — los filtros de fecha en los nuevos reportes usan los mismos URL params que el resto de reportes para compatibilidad con el componente compartido.
- **Transferencias entre sucursales (Fase P12 — 2026-04-17)** — Módulo nuevo en `/transferencias` (SELLER+MANAGER+ADMIN). **Schema aditivo** (migración `20260417075847_add_stock_transfer`): `StockTransfer` con cabecera + `StockTransferItem` polimórfico 4-way (`productVariantId` XOR `simpleProductId` XOR `batteryId` XOR `customerBikeId`, CHECK en SQL). `BatteryStatus` extendido con `IN_TRANSIT`. CHECKs adicionales: no self-transfer (`fromBranchId <> toBranchId`), cantidades válidas (`cantidadEnviada > 0`, `cantidadRecibida IS NULL OR 0 ≤ cantidadRecibida ≤ cantidadEnviada`). Enum `StockTransferStatus`: `SOLICITADA | BORRADOR | EN_TRANSITO | RECIBIDA | CANCELADA`. **Máquina de estados**: `SOLICITADA` (SELLER pull) → `BORRADOR` (MANAGER origen autoriza) → `EN_TRANSITO` (despacho) → `RECIBIDA` (terminal). `CANCELADA` accesible desde los tres estados no-terminales. MANAGER origen crea directo en `BORRADOR`; puede "enviar ahora" en un solo `$transaction`. **Regla de roles**: cualquier SELLER+ crea `SOLICITADA` hacia su branch; solo MANAGER+ADMIN transicionan a BORRADOR/EN_TRANSITO/CANCELADA (excepto auto-cancelación del creador en SOLICITADA). **Solo MANAGER+ADMIN del `toBranchId` reciben**. **Contabilidad de inventario**: despacho genera `InventoryMovement(TRANSFER_OUT, referenceId=transfer.id)` + decrement de Stock origen (variant/simple); `Battery.branchId` muta a destino con `status=IN_TRANSIT` al despachar (evita olvidar filtros "excluir en tránsito" en origen); `CustomerBike.branchId` muta al despachar arrastrando su Battery asignada (INSTALLED se conserva porque físicamente está montada). Recepción genera `InventoryMovement(TRANSFER_IN)` + increment Stock destino; **si `cantidadRecibida < cantidadEnviada` NO se crea ADJUSTMENT** — la merma queda visible en `StockTransferItem.cantidadRecibida < cantidadEnviada` y el neto contable ya está cubierto por `TRANSFER_OUT(n) + TRANSFER_IN(m) = -(n-m)` en origen. Battery pasa a `IN_STOCK`; CustomerBike no-op (ya está en destino). **Cancelación desde EN_TRANSITO** ejecuta reversa completa: `InventoryMovement(ADJUSTMENT)` positivo en origen (Stock upsert increment) + restore de `Battery`/`CustomerBike` a fromBranchId; cancelación desde SOLICITADA/BORRADOR es no-op de inventario. **Invariante contable**: todos los `InventoryMovement` de un transfer llevan `referenceId = transfer.id` — permite reconstruir la historia desde los movimientos. **Folio**: `TRF-{branchCode}-{seq}` con seq @@unique por `fromBranchId`, mismo patrón que ventas. **Integración POS (P12-A)**: `pos-terminal.tsx` recibe `remoteStockEntries` del Server Component padre (query paralela a `Stock` con `branchId: { not: session.branchId }` filtrada por IDs visibles). Icon `ArrowLeftRight` en cada card (variants + simples) abre `RemoteStockPopover`; cuando `stockLocal === 0 && remoteStock.length > 0`, el botón primario del card cambia a "Solicitar transferencia" (accent). Modal `RequestTransferDialog` crea siempre `SOLICITADA` desde el POS (SELLER pull); warning no-bloqueante de duplicados vía `GET /api/transferencias/solicitudes-activas` (guard: `targetBranchId === session.branchId` excepto ADMIN). **Desde el POS solo variants y simples**; baterías con serial y customer-bikes se solicitan desde `/transferencias` (modal polimórfico 4-way ya soporta los 4 tipos). Zero cambios en flujo de venta, carrito, caja, comisiones.

- **Rediseño del Taller Sub-fase A (Fase P13-A — 2026-04-17)** — Schema aditivo + APIs base del rediseño del módulo de taller. Brief: `docs/workshop-redesing/BRIEF.md`. **Schema** (migración `20260417162346_workshop_redesign_schema`, 100% aditiva, sin CHECKs manuales): enums `ChargeModel`, `ServiceOrderType`, `ServiceOrderSubStatus`, `ServiceOrderApprovalStatus`, `ServiceOrderApprovalChannel`; `ServiceOrder` +9 campos (`assignedTechId`, `servicedByUserId`, `type` default PAID, `subStatus`, `qaPassedAt/ByUserId`, `qaNotes`, `publicToken @unique`, `publicTokenEnabled`) + 4 índices; `ServiceCatalog.chargeModel`+`estimatedMinutes`; `ServiceOrderItem.laborMinutes`+`isExtra`+`createdAt`; `Branch.hourlyRate`+`whatsappTemplateTaller`; modelo nuevo `ServiceOrderApproval` (itemsJson snapshot). **Decisiones clave a no romper:** (1) Paths de API — los endpoints existentes `POST /api/workshop/orders`, `POST /api/workshop/orders/[id]/items`, `PATCH /api/workshop/orders/[id]/status` **no se movieron** (deuda cosmética para Fase 6); los endpoints nuevos (assign, sub-status, qa, approvals, public) cuelgan de `/api/service-orders/[id]/` junto con los ya existentes (charge/deliver/cancel). (2) **Vigencia de póliza no modelada hoy** — el enum `POLICY_MAINTENANCE` existe y `POST /api/workshop/orders` exige `customerBikeId` como guard mínimo, pero `assertPolicyActive(bikeId, tx)` en `src/lib/workshop.ts` es **no-op** con `console.warn` en los callers. La UI debe mostrar chip de advertencia, no bloquear. Cuando se modele la póliza, rellenar el cuerpo del helper sin breaking change de signature. (3) **Matriz de QA gating por tipo**: `assertQaPassed(order)` exige `qaPassedAt` para PAID/WARRANTY/POLICY_MAINTENANCE; **COURTESY es exenta** (revisión sin compromiso formal). El gate bloquea la transición a DELIVERED en `/deliver`. (4) **Matriz de cobro en `/deliver`**: PAID mantiene el flujo (caja + CashTransaction); WARRANTY/COURTESY/POLICY_MAINTENANCE crean `Sale(total=0)` sin CashTransaction (trazabilidad pura). Si el payload trae payment fields en tipos no-PAID, se ignoran con `console.warn` (no silent). El descuento de stock (WORKSHOP_USAGE) se mantiene en los 4 tipos. (5) **HOURLY pricing server-side**: `calculateHourlyPrice(Branch.hourlyRate, laborMinutes)` redondea a 2 decimales y se ejecuta en `POST /api/workshop/orders` y `POST /api/workshop/orders/[id]/items`. El cliente **nunca** dicta el precio de un catálogo HOURLY; si `Branch.hourlyRate` es null, 422 con mensaje en español. La convención para HOURLY es `quantity=1` con el tiempo en `laborMinutes`. (6) **`ServiceOrderItem.isExtra` = automático**: `false` si la orden está en PENDING (ítems del diagnóstico inicial); `true` si se agrega con status IN_PROGRESS o COMPLETED (post-diagnóstico). (7) **Cancelación auto-rechaza approvals PENDING** (`POST /api/service-orders/[id]/cancel` en la misma `$transaction`, respondedNote="Orden cancelada: {motivo}"). Evita approvals huérfanos visibles en el portal público. (8) **`ServiceOrderApproval.itemsJson` validado con Zod tanto al crear como al leer**: schema canónico en `src/lib/workshop-approvals.ts` (`approvalItemSchema`). Al APROBAR, `applyApprovalDecisionTx` materializa `ServiceOrderItem` con `isExtra=true` por cada línea del snapshot y limpia `subStatus=WAITING_APPROVAL`. Lógica compartida entre endpoint interno y público. (9) **Portal público** `GET /api/service-orders/public/[token]` sin auth; respuesta sanitizada estricta (folio, status, subStatus, type, bikeInfo, total, qaPassed boolean, approvals, sucursal con `phoneMasked` dejando los últimos 4, primer nombre del cliente). **Sin** `diagnosis`, `qaNotes`, ítems crudos, precios por línea, teléfono del cliente, datos fiscales. (10) **WhatsApp link nunca bloquea**: `POST /api/service-orders/[id]/approvals` retorna `{ approvalId, whatsappUrl, whatsappReason }`. `whatsappReason ∈ { "TEMPLATE_NOT_CONFIGURED", "CUSTOMER_HAS_NO_PHONE", null }`. El approval se crea siempre; la UI decide mostrar u ocultar el botón. Si `Branch.whatsappTemplateTaller` o `publicToken` faltan, reason = `TEMPLATE_NOT_CONFIGURED`. (11) **Trazabilidad de ejecución**: `PATCH /api/workshop/orders/[id]/status` autollena `servicedByUserId = session.userId` al entrar a COMPLETED (solo si null — primer ejecutor queda fijo). Reportes de productividad deben usar `servicedByUserId`, no `userId` (que sigue significando "quien registró"). (12) **Backfill publicToken**: `npx tsx prisma/backfill-workshop-tokens.ts` (idempotente, `where publicToken: null`). Seed transaccional (`prisma/seed-transactional.ts`) también genera `publicToken` y setea `qaPassedAt`/`qaPassedByUserId` cuando el target es DELIVERED para paridad dev con el gate real. **QA inmutable desde la API**: `POST /qa` es idempotente: si `qaPassedAt` ya está seteado, retorna 200 con estado actual e ignora el payload. Reversión queda para Fase 6 (endpoint administrativo dedicado). **Fuera de alcance de A**: UI (Kanban, wizard, ficha, drawer, portal, entrega) y PDFs del taller → sub-fases B-F. Nada de la UI actual (`NewOrderDialog`, `ServiceOrderDetails`, `WorkshopBoard`) se tocó; los nuevos campos opcionales garantizan backward compat del endpoint de create.

- **Rediseño del Taller Sub-fase B (Fase P13-B — 2026-04-21)** — Reescritura completa de `workshop-board.tsx` + expansión de queries en `page.tsx`. Commit: `b687807`. **Hallazgos de schema que difieren del BRIEF:** (1) `Capacidad.valorAh: Float` — el campo es `valorAh`, **no** `.valor` ni `.ah`. `computeBikeDisplay` usa `v.capacidad?.valorAh`. (2) `Voltaje.valor: Int` — correcto, sin cambios. (3) `CustomerBike` **NO tiene campo `bikeInfo`** — solo tiene `brand String?`, `model String?`, `color String?`, `voltaje String?`. El fallback de texto libre vive en `ServiceOrder.bikeInfo String?`. `computeBikeDisplay` en `page.tsx` recibe una `ActiveOrder` y usa `order.bikeInfo` (desde el modelo padre) como último fallback. (4) **No existe `fechaPromesa` en ningún modelo** — el aging de IN_PROGRESS usa `updatedAt + 48h` como proxy (spec §5 del BRIEF). (5) **No existe `@dnd-kit/core` en `package.json`** — se implementó HTML5 native DnD (`draggable`, `onDragStart`, `onDragOver`, `onDrop`, `useRef` para payload) como se especificó. **Arquitectura de archivo:** se creó `workshop-types.ts` como módulo neutral de tipos serializados para romper el ciclo de importación entre `page.tsx` y `workshop-board.tsx`. **React Compiler:** las sub-vistas `DesktopColumn` y `MobileSection` no pueden ser componentes internos — se convirtieron a funciones helper que retornan JSX (`renderColHeader`, `renderDesktopColBody`); la bandeja Pausada usa patrón IIFE `(() => { ... })()`. Cualquier nuevo componente que se necesite dentro de `WorkshopBoard` debe declararse en el top-level del módulo o en un archivo separado.

- **Rediseño del Taller Sub-fase C (P13-C — 2026-04-21)** — Wizard de recepción completo. Commits: `4906e28` (C.1 capa de datos) + `8e6edee` (C.2 UI). **Arquitectura:** Server Component `recepcion/page.tsx` con `force-dynamic` y prefetch paralelo (`getBranchMaintenanceServices` + `prisma.user.findMany` + `prisma.serviceCatalog.findMany`); prefetch adicional de `getBikeMaintenanceStatus` si `?customerBikeId=X` en URL. Client Component `recepcion-wizard.tsx` con `useForm<WizardFormData>` + `useReducer` para el step. Cada paso en archivo separado: `step-1-cliente.tsx`, `step-2-checklist.tsx`, `step-3-fotos.tsx`, `step-4-tipo.tsx`. **Compatibilidad zodResolver/RHF:** `@hookform/resolvers` v5 + Zod v4 generan conflicto de tipos en el genérico `TTransformedValues` del `Resolver`; la solución es `zodResolver(wizardSchema) as Resolver<WizardFormData>` (cast tipado, sin `any`). **Signature pad:** `signature_pad` v5.1.3 importado via `next/dynamic({ ssr: false })` en `step-2-checklist.tsx` para evitar mismatch de hidratación; el canvas se redimensiona en `useEffect` con `devicePixelRatio` para resolución correcta en Retina. **Upload fotos — patrón draft:** `POST /api/workshop/drafts/photos` escribe a `public/workshop/drafts/{userId}-{uuid}.webp` (sharp → WebP q85 1920px); el `userId` en el filename permite cleanup por usuario en Fase 6. `POST /api/workshop/orders` mueve los drafts a `public/workshop/{orderId}/` antes de la transacción via `moveDraftToOrder` (`fs.rename` atómico, fallback a `copyFile+unlink` si son volúmenes distintos); si la transacción falla, `cleanupOrderPhotos(orderId)` en el catch borra la carpeta. **Corrección:** la validación de `photoUrls` en `newOrderSchema` cambió de `startsWith("/public/workshop/")` → `startsWith("/workshop/drafts/")` para coincidir con las URLs reales de los drafts. **Deuda C.1 resuelta en C.2:** la generación de `orderId` se adelantó pre-transacción (con `crypto.randomUUID()`) para poder pasar `id: orderId` al `serviceOrder.create` y que el move de fotos sepa el destino antes de que la BD confirme. **IVA:** `Branch.ivaPct` no existe en el schema — IVA hardcodeado a 16% en `step-4-tipo.tsx`. **Diagnosis:** `POST /api/workshop/orders` tiene `z.string().min(1)` para diagnosis (no relajable en C.2 per scope); el wizard lo trata como campo requerido aunque el brief lo marca opcional. Resolver en C.3 si se quiere relajar. **Nuevas APIs auxiliares:** `GET /api/workshop/customers/search?q=` (búsqueda por nombre/RFC/teléfono, incluye bikes por sucursal) y `GET /api/workshop/bikes/[id]/maintenance-status` (llama `getBikeMaintenanceStatus`, serializa fechas como ISO strings).

- **P11 fusionado con /workshop (Fase P11 — 2026-04-17)** — El módulo de seguimiento de mantenimientos se integró como tab en `/workshop`, NO como módulo independiente en `/mantenimientos`. `layout.tsx` de workshop detecta `pathname` para activar el tab correcto; ambas rutas (`/workshop` y `/workshop/mantenimientos`) son Links de Next.js (SSR-friendly, bookmarkables). La regla de mantenimiento usa `ServiceCatalog.esMantenimiento` (flag explícito en BD) — **NO** matching por nombre del servicio. Un `ServiceOrder` solo cuenta como mantenimiento si `status = DELIVERED` Y al menos un `ServiceOrderItem` apunta a un `ServiceCatalog` con `esMantenimiento = true`. Los cálculos de semáforo (addMonths + diffDays) usan Date nativo — `date-fns` NO es una dependencia del proyecto. El botón "Crear orden de taller" linkea a `/customers/[id]` porque `NewOrderDialog` es un modal sin ruta URL propia; pre-población via query param queda como TODO P11.
...
| **P13** | **Rediseño del módulo de taller** — brief en `docs/workshop-redesing/BRIEF.md`, sub-fases A (schema+APIs) a F (portal público). Ver detalle en ROADMAP.md | Opus diseño → Sonnet impl | P1-C, P6 | 🟡 En curso |
| P13-A | Schema aditivo + APIs base (enums, ServiceOrderApproval, QA, sub-status, approvals, portal público GET/POST) | Opus | — | ✅ Completo (2026-04-17) |
| P13-B | Kanban rediseñado (7 columnas + Pausada lateral + aging + chips tipo/prepaid + filtros URL-sync + S5 bikeDisplay) | Sonnet | P13-A | ✅ Completo (2026-04-21) |
| P13-C | Wizard de recepción (4 pasos, firma canvas, uploader, asignación) | Sonnet | P13-A | ✅ Completo (2026-04-21) |
| P13-D | Ficha técnica rediseñada + drawer de aprobación + historial de bici | Sonnet | P13-A | ⏳ Pendiente |
| P13-E | Pantalla de entrega dedicada + PDFs del taller (entrada, etiqueta QR, estimate, comprobante) | Sonnet | P13-A, P13-D, P6 | ⏳ Pendiente |
| P13-F | Portal público del cliente (`/taller/public/[token]`, light mode forzado) | Sonnet | P13-A | ⏳ Pendiente |
...
1. Fix timezone global ✅ (2026-04-17) — 7 archivos migrados a `parseLocalDate`
1.5. Fix `fechaVencimiento` de `PurchaseReceipt` ✅ (2026-04-17) — migrado de `DateTime?` a `String?` "YYYY-MM-DD" (opción a). Ver §Decisiones clave y ROADMAP.md Paso 1.5
