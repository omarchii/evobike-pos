...
- **Reportes P10 Lote 6 (P10-G — 2026-04-17)** — Reporte agregado de compras al proveedor en `/reportes/compras-proveedor`. **Normalización en memoria**: `proveedor.trim().toLowerCase()` como key de agrupación; nombre canónico = el más frecuente del grupo (desempate alfabético). NO toca la BD — la normalización server-side en `POST /api/inventory/receipts` es deuda Fase 6. **⚠️ Deuda normalización**: la BD QA usa seeds consistentes, por lo que los casos de variación de case/espacios no se ejercitaron. Al cargar datos reales, verificar `/reportes/compras-proveedor` — si aparecen grupos duplicados, la normalización en `POST /api/inventory/receipts` pasa a alta prioridad. **⚠️ Deuda timezone**: `fechaVencimiento < now()` se evalúa en UTC server-side. En `America/Merida` (UTC−6) hay drift de 1 día en la franja 18:00–23:59 local — igual que el bug sistémico documentado en Lote 1. El fix es parte de la pasada global de timezone en Fase 6 (incluir los dos campos de vencimiento de P10-G al mismo tiempo).
